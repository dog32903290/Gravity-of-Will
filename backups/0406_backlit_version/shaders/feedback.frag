...
void main() {
  ...
  for (int ii = 0; ii < 24; ii++) { // Increased to 24
      ...
      float rim = smoothstep(0.0, dotSize, trueDist); // 越靠邊緣越透光 (Fresnel-like)
      float solidCore = mix(0.05, 3.5, backlitDiffuse * rim); // 產生逆光黑洞與月牙發光邊緣

      float intensity = dotShape * fade * (1.2 + emotionForce * 0.8) * solidCore;
  ...
  // 動態壓暗機制：白光越多，背景環境光越暗
  float ambientDimming = mix(1.0, 0.1, uParticleAmount); 
  plateShade *= ambientDimming;

  vec3 finalMudColor = vec3(plateShade) * cracks;

  // 圓心光源的放射全局照明 (Radial Center Light on Mud)
  float centerDistToUV = distFromCenter;
  float centerGlow = exp(-centerDistToUV * (5.5 - uEmotionIntensity * 2.0));
  vec3 spotLightColor = vec3(0.9, 0.6, 0.3) * centerGlow * (1.0 - cracks * 0.8) * mix(0.5, 2.0, uEmotionIntensity);
  finalMudColor += spotLightColor;

  // 逆光陰影：放射狀壓暗泥沼 (God Ray blocked by particles)
